...
import { parseAliExpressLinks } from '../../lib/utils.js';

// ============================
// Keep-Alive: maintain a long-lived port to the service worker
// so it doesn't go dormant during bulk listing operations.
// ============================
...
function startKeepAlive() {
  ...
  keepAlivePort = chrome.runtime.connect({ name: 'dropflow-keepalive' });
  keepAlivePort.onDisconnect.addListener(() => {
    keepAlivePort = null;
    // Reconnect if still running
    if (isRunning) {
      console.log('[DropFlow] Keep-alive disconnected, reconnecting...');
      setTimeout(startKeepAlive, 1000);
    }
  });
  // Ping every 5s to keep port active (aggressive keep-alive for Multilogin)
  keepAliveInterval = setInterval(() => {
    if (keepAlivePort) {
      try { keepAlivePort.postMessage({ type: 'ping' }); } catch(e) { /* port closed */ }
    }
    // Also send a message to wake SW if port is dead
    chrome.runtime.sendMessage({ type: 'KEEPALIVE_PING' }).catch(() => {});
  }, 5000);
  console.log('[DropFlow] Keep-alive started');
}

function stopKeepAlive() {
  if (keepAliveInterval) { clearInterval(keepAliveInterval); keepAliveInterval = null; }
  if (keepAlivePort) { try { keepAlivePort.disconnect(); } catch(e) {} keepAlivePort = null; }
  console.log('[DropFlow] Keep-alive stopped');
}
...
linksInput.addEventListener('input', () => {
  const links = parseAliExpressLinks(linksInput.value);
...
btnStart.addEventListener('click', async () => {
  const links = parseAliExpressLinks(linksInput.value);
  if (links.length === 0) {
    alert('Please paste at least one AliExpress product URL.');
    return;
  }
  ...
  const response = await chrome.runtime.sendMessage({ type: START_ALI_BULK_LISTING, ...payload });
...
btnPause.addEventListener('click', async () => {
  await chrome.runtime.sendMessage({ type: PAUSE_ALI_BULK_LISTING });
...
btnResume.addEventListener('click', async () => {
  await chrome.runtime.sendMessage({ type: RESUME_ALI_BULK_LISTING });
...
document.getElementById('btn-open-table').addEventListener('click', () => {
  const url = chrome.runtime.getURL('pages/ali-bulk-lister/status-table.html');
...
chrome.runtime.onMessage.addListener((message) => {
  switch (message.type) {
    case ALI_BULK_LISTING_PROGRESS:
      updateProgress(message);
      break;

    case ALI_BULK_LISTING_RESULT:
      addResult(message.result);
      break;

    case ALI_BULK_LISTING_COMPLETE:
      setRunningState(false);
      updateProgress(message);
      break;
  }

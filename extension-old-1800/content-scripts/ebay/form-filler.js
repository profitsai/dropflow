...
(function () {
  ...
  async function scrollPageToLoadAll() {
    ...
    await sleep(500);
    console.log('[DropFlow] Page scroll complete');
  }
  ...
  async function fillForm(productData) {
    ...
    try {
      ...
      // 3. Condition is already set during prelist/identify flow
      ...
      if (hasVariations) {
        ...
        const varResult = await fillVariations(productData);
      ...
      if (hasVariations && !results.variations) {
      ...
      // Hard safety: never submit a multi-variation product as single-SKU when
      // variation flow did not complete.
      if (hasVariations && !results.variations) {
        console.warn('[DropFlow] Variation flow incomplete; NOT submitting listing');
        await logVariationStep('fillVariations:blockSubmitAfterFailure', {});
        return results;
      }
  ...
  async function generateAIDescription(productData) {
    ...
    try {
      ...
      console.warn('[DropFlow] AI description failed:', response?.error);
    } catch (e) {
  ...
  async function fillVariations(productData) {
    ...
    // Filter to in-stock SKUs. If stock data is unavailable (all zeroes),
    // assume all are in stock â€” the product is listed on AliExpress so it's available.
    let inStockSkus = variations.skus.filter(s => s.stock > 0);
    if (inStockSkus.length === 0) {
      console.log('[DropFlow] All SKUs have stock=0, assuming all in stock (stock data may be unavailable)');
    ...
    const axesSummary = variations.axes.map(a => `${a.name}(${a.values.length})`).join(' Ã— ');
    ...
    if (preFoundEditBtn) {
      ...
      if (enabled === 'builder') {
        // Toggle triggered navigation to builder page
    ...
    for (let navWait = 0; navWait < 60; navWait++) {
      await sleep(300);
      ...
      // detectVariationBuilderContext() misses it because the iframe content
      // is still loading (textContentLen=0). Check directly each iteration.
      {
        const mskuFrame = document.querySelector('iframe[src*="bulkedit"]') ||
          document.querySelector('iframe[src*="msku"]');
        if (mskuFrame) {
          try {
            ...
            if (navWait % 10 === 0) {
              console.warn(`[DropFlow] MSKU iframe access error (iter=${navWait}): ${e.message}`);
            }
          }
        }
      }
      ...
      if (navWait === 10 || navWait === 24 || navWait === 40) {
    ...
    // =====================================================
    {
      const mskuIframe = document.querySelector('iframe[src*="bulkedit"]') ||
        document.querySelector('iframe[src*="msku"]');
      if (mskuIframe) {
    ...
    for (let attempt = 0; attempt < 35; attempt++) {
      ...
      {
        const mskuF = document.querySelector('iframe[src*="bulkedit"]') ||
          document.querySelector('iframe[src*="msku"]');
        if (mskuF) {
          try {
    ...
    // Find grid rows
    await sleep(1000);
    const gridRows = gridContext.querySelectorAll(
    ...
    for (const row of gridRows) {
      ...
      if (priceInputs.length === 0) continue;
    ...
    // Handle "Set all prices" shortcut if all prices are the same
    if (filledPrices === 0 && inStockSkus.length > 0) {
      const allPrices = inStockSkus.map(s => s.ebayPrice || s.price);
      const uniquePrices = [...new Set(allPrices)];
      if (uniquePrices.length === 1) {
        // Look for a bulk price input
        ...
        for (const input of bulkInputs) {
          ...
          if ((hint.includes('all') && hint.includes('price')) || hint.includes('bulk price') || hint.includes('set price')) {
            await commitInputValue(input, String(uniquePrices[0]));
            console.log(`[DropFlow] Set bulk price: ${uniquePrices[0]}`);
  ...
  async function runVariationBuilderPageFlow(productData, axisMapping = [], builderDoc = null) {
    ...
    // builder flow simultaneously for the same draftId.
    const draftIdMatch = window.location.href.match(/draftId=(\d+)/);
    const draftId = draftIdMatch ? draftIdMatch[1] : '';
    if (draftId) {
      const storageKey = `__dfBuilderLock_${draftId}`;
      try {
        const lockData = await chrome.storage.local.get(storageKey);
        const existing = lockData[storageKey];
        if (existing && (Date.now() - existing.ts) < 120000) {
          console.warn(`[DropFlow] Builder cross-context lock held (host=${existing.host}, age=${Date.now() - existing.ts}ms); skipping`);
          await logVariationStep('variationBuilder:crossContextLock', { host: existing.host, ageMs: Date.now() - existing.ts });
          return false;
        }
        await chrome.storage.local.set({ [storageKey]: { ts: Date.now(), host: window.location.hostname } });
      } catch (_) {}
      // Auto-clear lock after timeout (must match or exceed TTL of 120s)
      setTimeout(async () => {
    ...
    const ensureOptionSelected = async (axisName, value) => {
      ...
      if (opt) {
        if (!opt.selected) {
          // Use native .click() only — simulateClick double-fires which toggles
          // the option on then immediately off (same React issue as checkboxes).
          (asClickableTarget(opt.el)).click();
          await sleepShort();
          ...
          if (freshOpt && !freshOpt.selected) {
            // Retry with direct element click
            opt.el.click();
            await sleepShort();
          }
        }
    ...
    for (const mapped of mappedAxes) {
      ...
      const uniqueVals = Array.from(new Set(mapped.spec.axis.values)).slice(0, 40);
      console.warn(`[DropFlow] Axis "${mapped.spec.axis.name}" desired values (${uniqueVals.length}): [${uniqueVals.slice(0, 10).join(', ')}${uniqueVals.length > 10 ? '...' : ''}]`);
      let axisHits = 0;
    ...
    if (continueBtn && !continueBtn.disabled) {
      ...
      console.warn('[DropFlow] Variation builder flow completed and Continue clicked');
      return true;
    }
  ...
  async function ensureVariationsEnabled() {
    ...
    if (!closeBtn) {
      ...
      for (const btn of allBtns) {
        if (!isElementVisible(btn)) continue;
        const text = (btn.textContent || '').trim();
        // Match Ã—, âœ•, âœ–, X, or single-char close icons
        if (/^[Ã—âœ•âœ–xX\u00D7\u2715\u2716\u2717\u2718]$/.test(text)) {
          closeBtn = btn;
          break;
        }
  ...
  function findFileInput() {
    // #fehelix-uploader is eBay's specific file input ID on the listing form
    return document.querySelector('#fehelix-uploader') ||
           document.querySelector('input[type="file"][accept*="image"]') ||
           document.querySelector('input[type="file"][accept*=".jpg"]') ||
           document.querySelector('input[type="file"][accept*=".jpeg"]') ||
           document.querySelector('input[type="file"][multiple]') ||
           document.querySelector('[data-testid="image-upload"] input[type="file"]') ||
           document.querySelector('.photo-upload input[type="file"]') ||
           document.querySelector('[class*="photo"] input[type="file"]') ||
    ...
    try {
      ...
      if (!IS_TOP_FRAME) {
        ...
        const isLikelyBuilderFrame =
          ...
          /bulkedit\.ebay\./i.test(subframeHref) ||
          initialBuilderCtx.isBuilder;
        ...
        for (let poll = 0; poll <= maxPolls; poll++) {
          if (builderCtx.isBuilder) {
            ...
            await runVariationBuilderPageFlow(productData, [], builderCtx.doc || document);
            return;
          }
      ...
      if (initialBuilderCtx.isBuilder) {
        const hasVariations = !!productData.variations?.hasVariations;
        console.log(`[DropFlow] Found pending data on variation builder page (hasVariations=${hasVariations})`);
        if (hasVariations) {

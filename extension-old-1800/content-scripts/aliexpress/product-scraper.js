...
(function () {
  ...
  function scrapeVariationsFromDom() {
    ...
    let skuContainers = document.querySelectorAll(
      '[class*="sku-item"], [class*="sku-property"], [data-pl="product-sku"] > div, ' +
    ...
    for (const container of skuContainers) {
      ...
      // Find values
      const valueEls = container.querySelectorAll(
        'button, [class*="value"], [class*="item"]:not([class*="title"]), li, [role="option"], ' +
        '[class*="Value"], [class*="Item"]:not([class*="Title"]), a[class]'
      );
      const values = [];
      const seen = new Set();
      ...
      for (const valEl of valueEls) {
        ...
        // Try aria-label
        if (!valName) {
          valName = valEl.getAttribute('aria-label') || valEl.getAttribute('title') || '';
        }

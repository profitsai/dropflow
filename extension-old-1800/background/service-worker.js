...
// === Debug logging to storage (captures logs even when DevTools isn't attached) ===
const _logBuffer = [];
const _origConsole = { log: console.log.bind(console), warn: console.warn.bind(console), error: console.error.bind(console) };
function _flushLogs() {
  if (_logBuffer.length === 0) return;
  const batch = _logBuffer.splice(0, 200);
  chrome.storage.local.get({ _swLogs: [] }).then(({ _swLogs }) => {
    const logs = [..._swLogs, ...batch].slice(-500); // keep last 500
    chrome.storage.local.set({ _swLogs: logs });
  }).catch(() => {});
}
...
for (const level of ['log', 'warn', 'error']) {
  console[level] = (...args) => {
    ...
    _logBuffer.push({ t: Date.now(), l: level, m: args.map(a => typeof a === 'string' ? a : JSON.stringify(a)).join(' ') });
  };
}
...
function monitorEbayFormPage(tabId) {
  ...
  const listener = async (changeTabId, changeInfo, tab) => {
    ...
    // When the eBay tab navigates to the listing form page, force-inject
  ...
  const cleanupTimer = setTimeout(() => chrome.tabs.onUpdated.removeListener(listener), 180000);
  return () => {
    chrome.tabs.onUpdated.removeListener(listener);
 ...
 */
function waitForFormFilled(tabId, timeoutMs = 180000) {
  return new Promise((resolve, reject) => {
    const timer = setTimeout(() => {
      chrome.runtime.onMessage.removeListener(listener);
      reject(new Error('Form fill timed out (3 minutes)'));
    }, timeoutMs);
...
async function runBulkListing(links, options) {
  ...
  async function processLink(link, index) {
    ...
    try {
      ...
      const fillResult = await waitForFormFilled(ebayTabId, 180000);
      cleanupFormMonitor();
    ...
    try {
      ...
      try {
        productData = await Promise.race([
          ...
          new Promise((_, rej) => setTimeout(() => rej(new Error('Scrape timed out (20s)')), 20000))
        ]);
      } catch (firstErr) {
        ...
        productData = await Promise.race([
          chrome.tabs.sendMessage(aliTabId, { type: SCRAPE_ALIEXPRESS_PRODUCT }),
          new Promise((_, rej) => setTimeout(() => rej(new Error('Scrape retry timed out (20s)')), 20000))
        ]);
      }
      ...
      console.log(`[DropFlow Ali] Scraped: "${productData.title}" @ $${productData.price}`);
      ...
      try {
        ...
        // Storage quota exceeded — strip large base64 images and retry
        console.warn(`[DropFlow Ali] Storage.set failed (${storageErr.message}), retrying without preDownloadedImages...`);
